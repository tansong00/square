(function($) {
    ...
    var
        paneDiv = '<div id="pop_pane" class="square pie" style="position:fixed;z-index:11111;top:0;min-width:300px;margin:0;word-break:break-all;min-height:0;padding:0;left:50%;display:none;"></div>',
    ...
    $.fn.$pop = function(params) {
        if (params === 'out') {
            return popOut();
        }
        var cloneObj = $(this).clone(true, true);
        fnQueue.push(delay(cloneObj, params));
        if (!running) {
            running = true;
            pubSub.trigger('next');
        }
    ...
    $.$pop = function(text, action) {
        if (action == null) {
            $('<p style="text-align: center;padding: 5px 30px;">' + text + '</p>').$pop();
        }
        else if (action === 'out') {
            popOut();
        }
    ...
    function initPane(content, params) {
        var _$pane = $(paneDiv);
        if (params === 'text') {
            _$pane.css({maxWidth: '50%'});
        }
        _$pane.html(content);
